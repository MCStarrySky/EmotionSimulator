...
object CommandSet : CommandExecutor {
            ...
            player("player") {
                ...
                dynamic("value") {
                    executeAsCommandSender { sender, context, _ ->
                        ...
                        user.cast<Player>().set(abs(context["value"].toDoubleOrNull() ?: return@executeAsCommandSender))
                        sender.sendLang("command.subCommands.set.success", "now" to user.cast<Player>().getEmotion(), "before" to before)
                    }
                }
            }
        }
